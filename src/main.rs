...
use aws_sdk_cognitoidentityprovider::{
    error::SdkError,
    operation::admin_initiate_auth::{AdminInitiateAuthError, AdminInitiateAuthOutput},
    types::AuthFlowType,
};
use clap::Parser;
use colored::*;
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    //
    // {
    // 	"user_pool_id": "ap-southeast-XXXXXXXXXXXX",,
    ...
    let config_path = ".".to_string() + "/cognito_pool.json";
    ...
    match cognito_result {
        ...
        Err(e) => {
            eprintln!("An error occurred while authenticating: {:?}", e);
            return Err(e.into());
        }
    }
